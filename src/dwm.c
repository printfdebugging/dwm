...
static const Key keys[] = {
    ...
    {MODKEY | ShiftMask,   XK_p,      spawn,          SHCMD("cred --add-qr")  },
    {MODKEY | ControlMask, XK_p,      spawn,          SHCMD("cred --gen-pass")},
        ...
        for (i = 0; i < LENGTH(buttons); i++)
            if (buttons[i].click == ClkClientWin)
                for (j = 0; j < LENGTH(modifiers); j++)
                    XGrabButton(
                        dpy, buttons[i].button, buttons[i].mask | modifiers[j], c->win, False, BUTTONMASK, GrabModeAsync, GrabModeSync, None, None);
    }
}
...
pid_t winpid(Window w) {
    ...
    if (XGetWindowProperty(dpy, w, XInternAtom(dpy, "_NET_WM_PID", 0), 0, 1, False, AnyPropertyType, &type, &format, &len, &bytes, &prop) !=
            Success ||
        !prop)
        return 0;
